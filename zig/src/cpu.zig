...
pub const CPU = struct {
    ...
    fn getOpCode(self: *Self) u16 {
        const opcode = @as(u16, @intCast(self.memory.get(self.pc))) << 8 | self.memory.get(self.pc + 1);
        self.pc += 2;
        return opcode;
    }

    fn execute(self: *Self, opcode: u16, display: *Display, keyboard: *Keyboard) i32 {
        const x: u8 = @intCast(opcode >> 8 & 0x0F);
        const y: u8 = @intCast(opcode >> 4 & 0x0F);
        const n: u8 = @intCast(opcode & 0x000F);
    ...
    fn opDisplay(self: *Self, x: u8, y: u8, n: u8, display: *Display) u16 {
        ...
        const vx: u16 = @intCast(self.registers[x]);
        const vy: u16 = @intCast(self.registers[y]);
        ...
        while (row < vy + @as(u16, @intCast(n))) : (row += 1) {
            ...
            const startX = vx + row * 64;
            const endX = startX + 8;
            var bitMask: u8 = 0x80;
            var col = startX;
            while (col < endX) : (col += 1) {
                if ((col % 64) >= (startX % 64)) {
                    const position = (row * 64) + (col % 64);
    ...
    fn opRandom(self: *Self, x: u16, nn: u8) u16 {
        const random = rnd.random().int(u8);
    ...
    fn opShiftRight(self: *Self, x: u16) u16 {
        const flag = self.registers[x] & 0x1;
        self.registers[x] >>= 1;
    ...
    fn opShiftLeft(self: *Self, x: u16) u16 {
        const flag = (self.registers[x] & 0x80) >> 7;
        self.registers[x] <<= 1;
        self.registers[0xF] = flag;
        return 200;
    }
    ...
    fn opAddWithCarry(self: *Self, x: u8, y: u8) u16 {
        const sum = @as(u16, @intCast(self.registers[x])) + @as(u16, @intCast(self.registers[y]));
        if (sum > 255) {
...
//     var data: [4]u8 = [_]u8{ 0x12, 0x02, 0x60, 0x01 };
